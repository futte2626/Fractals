...
public class PaperSplitRectangleRender implements FractalRenderer {
    ...
    private void checkRectangle(IntPoint topLeft, IntPoint bottomRight, boolean verticalSplit, SceneSettings options, int maxIterations, ColorScheme colorScheme) {
        ...
        for (int x = tlx; x <= brx; x++) {
            ...
            image.setRGB(x, tly, 255);
            image.setRGB(x, bry, 255);
        ...
        if (sameIterationCount) {
            for (int y = tly + 1; y <= bry - 1; y++) {
                ...
                image.setRGB(tlx, y, 255);
                image.setRGB(brx, y, 255);
